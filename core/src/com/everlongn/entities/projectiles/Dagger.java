...
public class Dagger extends Throw {
    ...
    public Dagger(float x, float y, int direction, float angle, float damage, Entity source) {
        ...
        throwBound = new Rectangle(0, 0, Throwing.dagger.width, Throwing.dagger.height);
    }

    @Override
    public void tick() {
        throwBound.setPosition(body.getPosition().x*Constants.PPM+2 - Throwing.dagger.width/2, body.getPosition().y*Constants.PPM+2 - Throwing.dagger.height/2);
    ...
    public void checkPickedUp() {
        ...
        if(collected) {
            ...
            if(throwBound.overlaps(Player.itemCollectBound)) {
                GameState.inventory.addItem(Throwing.dagger.createNew(1));
                pickedUp = true;
                deactivate = true;
            }
        } else {
